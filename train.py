...
model = init_model(len(data), len(vocab), n_topics, l1_doc, l1_word, lr)
...
for epoch in range(0, n_epochs):
	...
	print("\tL2/L1\t>2/N\tL2/L1")
	doc_sparsity = sparsity(model.layers[docvec_layerN])
	word_sparsity = sparsity(model.layers[wordvec_layerN])
	...
	log['a_Epoch'], log['m_DocL2L1'], log['m_WordL2L1'], log['m_DocAbove2N'] = epoch, doc_sparsity, word_sparsity, above2N
	topic_words = get_topic_words()
	log['m_tOverlap'] = topic_overlap(wordvecs, topic_words)
	log['m_tPrec'],	log['m_tRecall'] = topic_prec_recall(wordvecs, topic_words, cntr, stopidxs=set())#[token2idx[w] for w in stopwords])
	...
	write_log(log, "log_stt201712.csv")
